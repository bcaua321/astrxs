...
export default function ApodContent({ currentDate } : { currentDate : string  } ) {
    ...
    //             </div>
    //             {data.media_type === "video" && <iframe allowFullScreen src={data.url} />}
    ...
    return (
        <>
            <div className="flex flex-col justify-center items-center mt-5 w-[95%] h-full max-h-max">
                <div className="flex justify-center items-center mb-4 cursor-pointer" onClick={onOpen}>
                    <h2 className="m-2 text-center font-bold text-[0.8rem] md:text-lg">
                        {data.title}  
                    </h2>
                ...
                {data.media_type === "video" && (
                    <iframe className="w-full aspect-video mb-10" src={data.url} title="YouTube video player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen></iframe>
                )}
            </div>
